...
import type { Product, ProductPayload, ProductFilterParams } from "../../types/product";
...
const product = {
  ...
  mutations: {
    ...
    setProducts(state: ProductState, payload: { products: Product[], total?: number, currentPage?: number, totalPages?: number }) {
      state.products = payload.products;
      if (payload.total !== undefined) state.totalProducts = payload.total;
      if (payload.currentPage !== undefined) state.currentPage = payload.currentPage;
      if (payload.totalPages !== undefined) state.totalPages = payload.totalPages;
    },
  ...
  actions: {
    async fetchProducts({ commit }: { commit: Commit }, params: ProductFilterParams = {}) {
    ...
    async createProduct({ commit }: { commit: Commit }, productData: ProductPayload) {
      commit("setLoading", true);
      commit("setError", null);
      ...
      try {
        ...
        const errorMsg = error.response?.data?.message || 'Không thể tạo sản phẩm';
    ...
    async updateProduct({ commit }: { commit: Commit }, { id, data }: { id: number, data: ProductPayload }) {
      commit("setLoading", true);
      commit("setError", null);
  ...
  getters: {
    ...
    productById: (state: ProductState) => (id: number): Product | undefined => 
      state.products.find(p => p.id === id),
    currentProduct: (state: ProductState): Product | null => state.currentProduct,
